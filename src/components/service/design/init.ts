 ...
 * DESIGN page engine.
 *
 * The page carries no scroll set-piece of its own — the ad assembly in #craft
 * is driven entirely by CSS off the section's `.in` class, and the work strip
 * is a marquee — so this is only the shared furniture: the section rail, the
 * progress line, and the fixed background that takes its colour from whichever
 * section is under the middle of the viewport.
 */
